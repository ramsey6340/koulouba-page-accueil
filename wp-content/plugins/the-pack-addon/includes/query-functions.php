...
{
    return [
        'tag' => esc_html__('Tag', 'thepack'),
        'cat' => esc_html__('Category color', 'thepack'),
        'cat_bg' => esc_html__('Category background color', 'thepack'),
        'title' => esc_html__('Post title & permalink', 'thepack'),
        'title_nolink' => esc_html__('Post title no permalink', 'thepack'),
        'title_single' => esc_html__('Single post title', 'thepack'),
        'time' => esc_html__('Date', 'thepack'),
        'share' => esc_html__('Share button', 'thepack'),
        'thumb' => esc_html__('Thumbnail', 'thepack'),
        'excerpt' => esc_html__('Excerpt', 'thepack'),
        'space' => esc_html__('Space with separator', 'thepack'),
        'clear' => esc_html__('Clear section', 'thepack'),
        'clear10' => esc_html__('Clear with 10px width', 'thepack'),
        'h5' => esc_html__('Clear with 5px height', 'thepack'),
        'h10' => esc_html__('Clear with 10px height', 'thepack'),
        'w10' => esc_html__('Width 10px', 'thepack'),
        'hr' => esc_html__('Horizontal line', 'thepack'),
        'btn' => esc_html__('Readmore Button', 'thepack'),
        'author' => esc_html__('Author', 'thepack'),
        'comment' => esc_html__('Comment No', 'thepack'),
    ];
}
...
{
    return [
        'none' => esc_html__('No order', 'thepack'),
        'ID' => esc_html__('Post ID', 'thepack'),
        'author' => esc_html__('Author', 'thepack'),
        'title' => esc_html__('Title', 'thepack'),
        'date' => esc_html__('Published date', 'thepack'),
        'modified' => esc_html__('Modified date', 'thepack'),
        'parent' => esc_html__('By parent', 'thepack'),
        'rand' => esc_html__('Random order', 'thepack'),
        'comment_count' => esc_html__('Comment count', 'thepack'),
        'menu_order' => esc_html__('Menu order', 'thepack'),
        'post__in' => esc_html__('By include order', 'thepack'),
    ];
}
...
function thepack_builder_comment()
{
    if (!post_password_required()) {
        ...
        if (comments_open()) {
            if ($num_comments == 0) {
                $comments = esc_html__('0 Comment', 'thepack');
            } elseif ($num_comments > 1) {
                $comments = $num_comments . esc_html__(' Comments', 'thepack');
            } else {
                $comments = esc_html__('1 Comment', 'thepack');
            }
            $write_comments = $comments;
        } else {
            $write_comments = esc_html__('off', 'thepack');
        }
...
function thepack_builder_reading_time()
{
    ...
    if ($word_count >= 1) {
        $out = sprintf(_n('%d min', '%d min', $word_count, 'thepack'), $word_count);
...
<<?php echo esc_attr($tag); ?><?php comment_class(empty($args['has_children']) ? '' : 'parent') ?>
    ...
    <div id="div-comment-<?php comment_ID() ?>" class="article">
        ...
        <div class="details">
            ...
            <?php printf(__('<h4 class="user-name">%s</h4>', 'thepack'), get_comment_author_link()); ?>
            <span class="user-date"><?php printf(__('%1$s', 'thepack'), get_comment_date()); ?></span>

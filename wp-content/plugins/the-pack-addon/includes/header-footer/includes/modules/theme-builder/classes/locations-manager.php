...
class Locations_Manager {
	...
	public function enqueue_styles() {
		...
		if ( ! empty( $css_files ) ) {
			thepack_kit()->elementor()->frontend->enqueue_styles();
	...
	public function template_include( $template ) {
		...
		if ( is_singular() ) {
			$document = thepack_kit()->elementor()->documents->get_doc_for_frontend( get_the_ID() );
		 ...
		 */
		$page_templates_module = thepack_kit()->elementor()->modules_manager->get_modules( 'page-templates' );
	...
	public function do_location( $location ) {
        ...
        $documents_by_conditions = apply_filters('thepack-kit/prepare-do-location', null, $location);
	...
	public function inspector_log( $args ) {
		$inspector_enabled = method_exists( thepack_kit()->elementor()->inspector, 'is_enabled' ) && thepack_kit()->elementor()->inspector->is_enabled();
		...
		if ( isset( $args['template'] ) ) {
			$title[] = thepack_kit()->elementor()->inspector->parse_template_path( $args['template'] );
		}

		$title = implode( ' > ', $title );

		thepack_kit()->elementor()->inspector->add_log( 'Theme', $title, $url );
	}
